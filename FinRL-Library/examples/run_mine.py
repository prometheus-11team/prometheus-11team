...
# 주식 데이터 불러오기
df_stock = pd.read_csv("/Users/gamjawon/FinRL-Library/examples/data/M7_financial_data_2020_2025.csv")
df_stock["date"] = pd.to_datetime(df_stock["date"])
df_stock = df_stock.rename(columns={"ticker": "tic"})
df_stock = df_stock.sort_values(["date", "tic"]).reset_index(drop=True)

# 거시경제 지표 불러오기
df_macro = pd.read_csv("/Users/gamjawon/FinRL-Library/examples/data/macro_indicators_2020_2024.csv")
...
env_kwargs = {
    ...
    "state_space": 19,
    "stock_dim": stock_dim,
    "tech_indicator_list": ALL_INDICATORS + ['transformer_pred', 'transformer_conf', 
                                            'sentiment_score', 'sentiment_pos', 
                                            'sentiment_neg', 'sentiment_conf'],  # 모든 지표 사용
...
# 모델 저장
model_save_path = os.path.join("models", "TD3_model_update")
